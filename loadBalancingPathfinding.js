...
function findShortestPath(startNodeName, endNodeName,updateLoad) {
    ...
    path.forEach((node, index) => {
        if (index < path.length - 1) {
            ...
            if (connection) {
                ...
                // Highlight the line
                if (connection.line) {
                    connection.line.style.backgroundColor = "red"; // Change color to red
                    // Revert the line color back to black after 1.5 seconds
                    setTimeout(() => {
                        connection.line.style.backgroundColor = "black";
                    }, 4000);
                } else {
                    console.warn("Line element not found for connection");
                }
            } else {
